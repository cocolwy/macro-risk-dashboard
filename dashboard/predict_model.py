...
def get_historical_fomc_dates():
    """FOMC statement release dates 1994-2015 (pre-Lucca & Moench publication era)."""
    dates = [
        # 1994
        '1994-02-04', '1994-03-22', '1994-04-18', '1994-05-17', '1994-08-16',
        '1994-09-27', '1994-11-15', '1994-12-19',
        # 1995
        ...
        '1998-09-29', '1998-10-15', '1998-11-17', '1998-12-22',
        # 1999
        ...
        # 2001 (incl. emergency cuts post-9/11)
        ...
        # 2008 (incl. emergency cuts during GFC)
...
def get_event_dates():
    """Known FOMC meeting dates, CPI release dates (2016-2026). NFP = 1st Friday of month."""
    fomc = [
...
def build_features_events_slim(df: pd.DataFrame) -> pd.DataFrame:
    """Slim features + only the top 4 event features (remove redundant ones)."""
    features = build_features_slim(df)
    event_feats = build_event_features(df)
    keep = ['fomc_days_since', 'fomc_days_to', 'cpi_within_3d', 'nfp_within_3d']
    for col in keep:
        if col in event_feats.columns:
            features[col] = event_feats[col]
    return features


def build_features_events_decay(df: pd.DataFrame) -> pd.DataFrame:
    """Slim features + exponential decay transforms on event distances."""
    features = build_features_slim(df)
    event_feats = build_event_features(df)
    for event_name in ['fomc', 'cpi']:
        dt = event_feats[f'{event_name}_days_to']
        ds = event_feats[f'{event_name}_days_since']
        features[f'{event_name}_urgency'] = np.exp(-dt / 5.0)
        features[f'{event_name}_recency'] = np.exp(-ds / 5.0)
    features['nfp_within_3d'] = event_feats['nfp_within_3d']
    return features
